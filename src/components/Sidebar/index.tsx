...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  const filteredMenuItems = menuGroups[0].menuItems.filter(
    (item) => item.visibleForCreator === true,
  );
    ...
    <ClickOutside onClick={() => setSidebarOpen(false)}>
      ...
      >
        ...
        <div className="flex items-center justify-between gap-2 px-6 py-5.5 lg:py-6.5">
          <Link
            className="text-xl font-semibold tracking-wide text-blue-600 transition-colors duration-300 hover:text-blue-800"
            href="/"
          >
            Job Portal
          </Link>
              ...
              <div key={groupIndex}>
                ...
                <ul className="mb-6 flex flex-col gap-1.5">
                  {group.menuItems.map((menuItem, menuIndex) => {
                    const isVisibleForCreator = menuItem.visibleForCreator;
                    if (userCreator) {
                      if (
                        isVisibleForCreator === true ||
                        typeof isVisibleForCreator === "undefined"
                      ) {
                        return (
                          <SidebarItem
                            key={menuIndex}
                    ...
                    // Return null if the condition is not met (this ensures that no item is rendered if conditions are not matched)
                    return null;
                  })}
                </ul>
              </div>
